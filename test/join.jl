module TestJoin
    ...
    right = outer[map(x->!isnull(x), outer[:Job]), [:Name, :ID, :Job]]
    left = outer[map(x->!isnull(x), outer[:Name]), :]
    inner = left[map(x->!isnull(x), left[:Job]), :]
    semi = unique(inner[:, [:ID, :Name]])
    anti = left[map(isnull, left[:Job]), [:ID, :Name]]
